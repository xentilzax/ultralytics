...
исходные датасет представлен в виде структуры: 
source_dataset/
├── images/
│   ├── train/
│   │   ├── img1.jpg
...
В репозитории представленны результаты обучения 
result/
├── train  (10  epoch, 640x640)
├── train2 (100 epoch, 640x640)
├── train3 (100 epoch, предварительная аугментация, так же включена дефалтная аументация в ultralitics, batch=16, 360x640) задан не корректный размер входного слоя, должен быть кратный 32
├── train4 (100 epoch, предварительная аугментация, так же включена дефалтная аументация в ultralitics, batch=32, 360x640) задан не корректный размер входного слоя, должен быть кратный 32
├── train5 (100 epoch, предварительная аугментация, batch=32, 360x640) задан не корректный размер входного слоя, должен быть кратный 32
